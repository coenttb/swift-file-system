...
#if os(Windows)
extension File.System.Link.Hard {
    internal static func _createWindows(at path: File.Path, to existing: File.Path) throws(Error) {
    ...
    private static func _mapWindowsError(_ error: DWORD, path: File.Path, existing: File.Path) -> Error {
        switch error {
