...
#if !os(Windows)
extension File.System.Link.Symbolic {
    internal static func _createPOSIX(at path: File.Path, pointingTo target: File.Path) throws(Error) {
...
#if os(Windows)
extension File.System.Link.Symbolic {
    internal static func _createWindows(at path: File.Path, pointingTo target: File.Path) throws(Error) {
        ...
        if targetAttrs != INVALID_FILE_ATTRIBUTES && (targetAttrs & FILE_ATTRIBUTE_DIRECTORY) != 0 {
