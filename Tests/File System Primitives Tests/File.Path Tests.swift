...
extension File.System.Test.Unit {
    ...
    struct Path {
        ...
        func parentChain() throws {
            let path = try File.Path.init("/a/b/c")
        ...
        func appendingPath() throws {
            let base = try File.Path.init("/usr")
            let suffix = try File.Path.init("local/bin")
        ...
        func extensionOfFileWithMultipleDots() throws {
            let path = try File.Path.init("/tmp/file.tar.gz")
        ...
        func isAbsoluteForAbsolutePath() throws {
            let path = try File.Path.init("/usr/bin")
        ...
        func slashOperatorWithString() throws {
            let path = try File.Path.init("/usr")
        ...
        func hashableConformance() throws {
            let path1 = try File.Path.init("/usr/local")
            let path2 = try File.Path.init("/usr/local")
            let path3 = try File.Path.init("/usr/bin")
        ...
        func useAsDictionaryKey() throws {
            let path1 = try File.Path.init("/usr/local")
            let path2 = try File.Path.init("/usr/bin")
