...
extension File.System.Test.EdgeCase {
    ...
    func handleOperationsBeforeClose() throws {
        ...
        let filePath = try File.Path(path)
        var handle = try File.Handle.open(filePath, mode: .readWrite, options: [.create, .closeOnExec])
    ...
    func writeToReadOnlyHandle() throws {
        ...
        let filePath = try File.Path(path)
        var createHandle = try File.Handle.open(filePath, mode: .write, options: [.create, .closeOnExec])
    ...
    func multipleHandles() throws {
        ...
        var handle1 = try File.Handle.open(filePath, mode: .readWrite, options: [.create, .closeOnExec])
        let data: [UInt8] = [1, 2, 3, 4, 5]
        try data.withUnsafeBufferPointer { buffer in
    ...
    func moveOverExisting() throws {
        ...
        var srcHandle = try File.Handle.open(srcPath, mode: .write, options: [.create, .closeOnExec])
        ...
        var dstHandle = try File.Handle.open(dstPath, mode: .write, options: [.create, .closeOnExec])
